...
const Playlists = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch Playlists
      try {
        const playlistsRes = await fetch(`${API_URL}/api/admin/playlists`, { headers });
        if (playlistsRes.ok) {
      ...
      // Fetch Players
      try {
        const playersRes = await fetch(`${API_URL}/api/admin/players`, { headers });
        if (playersRes.ok) {
  ...
  useEffect(() => {
    ...
    if (previewing && playlistItems.length > 0) {
      ...
      const duration = currentItem.duration > 0 ? currentItem.duration * 1000 : 5000; // fallback 5s for videos if duration is 0
      ...
      <div className="space-y-6 animate-in fade-in duration-300">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Construct your content sequence</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition shadow-sm font-medium disabled:opacity-50"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
              <div className="flex justify-between items-center">
                <h2 className="font-semibold text-gray-900">Player Assignment</h2>
                <button 
                ...
                {selectedPlayers.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedPlayers.map(id => {
                      ...
                      return (
                        <span key={id} className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-medium border border-blue-100">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="font-semibold text-gray-900">Content Sequence ({playlistItems.length})</h2>
              <button 
                onClick={() => setShowMediaPicker(true)}
                className="flex items-center gap-2 text-sm bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition font-medium"
              >
                ...
                playlistItems.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 uppercase">{item.type.split('/')[1] || item.type}</p>
                    </div>
                    <div className="flex items-center gap-4">
            ...
            {playlistItems.length > 0 && (
              <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-blue-800">
                  {Math.floor(playlistItems.reduce((acc, item) => acc + (item.duration || 0), 0) / 60)}m {playlistItems.reduce((acc, item) => acc + (item.duration || 0), 0) % 60}s
                </span>
              </div>
            )}
          </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {mediaLibrary.map(media => (
                    ...
                    >
                      ...
                      <div className="p-3">
                        <p className="text-xs font-bold text-gray-900 truncate">{media.name}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {showPlayerAssign && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-xl font-bold text-gray-900">Assign Players</h3>
                <button onClick={() => setShowPlayerAssign(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-6 space-y-2 max-h-[60vh] overflow-y-auto">
                {players.length === 0 ? (
                  ...
                  players.map(player => (
                    <label key={player.id} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:bg-gray-50 cursor-pointer transition">
                      <input 
                        type="checkbox"
                      ...
                      <div className={`w-2 h-2 rounded-full ${player.status === 'online' ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                    </label>
                  ))
                )}
              </div>
          ...
          <div className="fixed inset-0 z-[60] flex flex-col bg-black text-white p-8">
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-gray-400 text-sm">Previewing Item {currentPreviewIndex + 1} of {playlistItems.length}</p>
              </div>
              <button 
              ...
              {playlistItems.map((_, idx) => (
                <div 
                  ...
                  className={`h-1 flex-1 max-w-[40px] rounded-full transition-all duration-300 ${
                    idx === currentPreviewIndex ? "bg-blue-500 scale-y-150" : "bg-white/20"
                  }`}
                />
              ))}
            </div>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage content loops and schedules.</p>
        </div>
        <button 
          onClick={startNewPlaylist}
          className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2 rounded-xl hover:bg-blue-700 transition shadow-md font-bold"
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-xl shadow-sm border border-gray-100 border-dashed">
          ...
          <h3 className="text-xl font-medium text-gray-900">No playlists found</h3>
          <p className="text-gray-500 mt-2">Create your first playlist to start broadcasting content.</p>
          <button 
            onClick={startNewPlaylist}
            className="mt-6 px-6 py-2 bg-blue-50 text-blue-600 rounded-lg font-bold hover:bg-blue-100 transition"
          >
            Get Started
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPlaylists.map(playlist => (
            <div key={playlist.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg transition-all group overflow-hidden flex flex-col">
              <div className="p-6 flex-1">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-1">{playlist.name}</h3>
                <p className="text-sm text-gray-500 line-clamp-2 mb-6">{playlist.description || "No description provided."}</p>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center gap-2 text-gray-600 bg-gray-50 p-2 rounded-lg border border-gray-100">
                    <Film className="w-4 h-4 text-blue-500" />
                    <span className="text-xs font-bold">{playlist.items?.length || 0} Items</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-600 bg-gray-50 p-2 rounded-lg border border-gray-100">
                    <Clock className="w-4 h-4 text-orange-500" />
                    <span className="text-xs font-bold">
                      {Math.floor((playlist.items?.reduce((acc, item) => acc + (item.duration || 0), 0) || 0) / 60)}m { (playlist.items?.reduce((acc, item) => acc + (item.duration || 0), 0) || 0) % 60}s
                    </span>
                  </div>
                </div>
              </div>
